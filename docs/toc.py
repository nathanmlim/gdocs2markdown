...
def directory_line(file_name, full_path, level):
    ...
    return ('\t' * level) + '- ' + (file_name
                                    if not has_readme else '[%s](%s)' % (file_name, urllib.pathname2url(full_path)))
...
parser.add_argument(
    "--readme",
    help=
    "Searches and replaces the lines between toc-start and toc-end in the given file and prints the output. If not given, the script just prints the TOC generated."
)
